...
data Expr : Set where
  ...
  -- ifz_[zero↦_,succ_↦_] : Expr → Expr → Name → Expr → Expr


  -- System FPC
  fold⟦_is_⟧_ : Name → Type → Expr → Expr
  unfold : Expr → Expr
...
[_/_]ₜ_ : Type → Name → Type → Type
[ e' / x ]ₜ num = num
[ e' / x ]ₜ str = str
[ e' / x ]ₜ (e₁ ⇒ e₂) =  ([ e' / x ]ₜ e₁) ⇒ ([ e' / x ]ₜ e₂)
[ e' / x ]ₜ unit = unit
[ e' / x ]ₜ (e₁ ⊗ e₂) = ([ e' / x ]ₜ e₁) ⊗ ([ e' / x ]ₜ e₂)
[ e' / x ]ₜ (e₁ ⊕ e₂) = ([ e' / x ]ₜ e₁) ⊕ ([ e' / x ]ₜ e₂)
[ e' / x ]ₜ [ y ] with x ≟string y
([ e' / x ]ₜ [ y ]) | yes p =  e'
([ e' / x ]ₜ [ y ]) | no ¬p =  [ y ]
[ e' / x ]ₜ (rec y is e) with x ≟string y
...| yes x≡y =  rec y is e
...| no x≠y  =  rec y is ( [ e' / x ]ₜ e)
...
data _⊢_∶_ : Context → Expr → Type → Set where
  ...
  T-fold : ∀ {Γ t τ e}
         → Γ ⊢ e ∶ ([ (rec t is τ) / t ]ₜ τ)
         ---------------------------------------------
         → Γ ⊢ fold⟦ t is τ ⟧( e ) ∶ (rec t is τ)

  T-unfold : ∀ {Γ e t τ}
           → Γ ⊢ e ∶ (rec t is τ)
           → Γ ⊢ unfold e ∶ ( [  rec t is τ / t ]ₜ τ)
...
[ x ] ≟ [ y ] with x ≟string y
...| yes refl = yes refl
...| no x≠y   = no λ x → impossible
(rec t₁ is τ₁) ≟ (rec t₂ is τ₂) with (t₁ ≟string t₂) , (τ₁ ≟ τ₂)
...| yes refl , yes refl =  yes refl
{-# CATCHALL #-}
...
typeinfer Γ case e [left x ↦ e₁ ,right y ↦ e₂ ] | just (τ ⇒ τ₁ , ⊢e) =  nothing
typeinfer Γ case e [left x ↦ e₁ ,right y ↦ e₂ ] | just ([ a ] , ⊢e) =  nothing
typeinfer Γ case e [left x ↦ e₁ ,right y ↦ e₂ ] | just ((rec t is τ) , ⊢e) =  nothing
...
typeinfer Γ (fix x ∶ τ is e) with typeinfer (( x , τ) ∷ Γ) e
typeinfer Γ (fix x ∶ τ is e) | nothing = nothing
typeinfer Γ (fix x ∶ τ is e) | just (τ' , ⊢e) with τ ≟ τ'
typeinfer Γ (fix x ∶ τ is e) | just (τ' , ⊢e) | yes refl =  just ( τ ,  T-fix  ⊢e)
typeinfer Γ (fix x ∶ τ is e) | just (τ' , ⊢e) | no ¬p =  nothing

typeinfer Γ (fold⟦ t is τ ⟧ e) with typeinfer Γ e
typeinfer Γ (fold⟦ t is τ ⟧ e) | nothing =  nothing
typeinfer Γ (fold⟦ t is τ ⟧ e) | just (tₑ , ⊢e) with tₑ ≟ ( [ rec t is τ / t ]ₜ τ)
typeinfer Γ (fold⟦ t is τ ⟧ e) | just (tₑ , ⊢e) | yes refl = just (rec t is τ , T-fold  ⊢e)
typeinfer Γ (fold⟦ t is τ ⟧ e) | just (tₑ , ⊢e) | no neq =  nothing
typeinfer Γ (unfold e) with typeinfer Γ e
typeinfer Γ (unfold e) | nothing = nothing
typeinfer Γ (unfold e) | just ((rec t is τ), ⊢e) =  just ([ (rec t is τ) / t ]ₜ τ ,  T-unfold ⊢e)
{-# CATCHALL #-}
typeinfer Γ (unfold e) | just (τ , ⊢e) =  nothing
...
[ e' / x ] (fold⟦ t is τ ⟧ e) =  fold⟦ t is τ ⟧ ([ e' / x ] e)
[ e' / x ] (unfold e) = unfold ([ e' / x ] e)
...
data Canonical : Expr → Type → Set where
  ...
  C-fold : ∀ {e t τ}
         → Canonical e ([(rec t is τ) / t ]ₜ τ)
         → Canonical (fold⟦ t is τ ⟧ e) (rec t is τ)
...
canonical (T-fold ⊢e) (V-fold V-e) with canonical ⊢e V-e
...| C-e =  C-fold  C-e
...
value (C-inj₂ C-e) = V-inj₂ (value C-e)
value (C-arr ⊢e) =  V-lam
value (C-fold C-e) =  V-fold (value C-e)
...
typed (C-fold C-e) =  T-fold (typed C-e)
